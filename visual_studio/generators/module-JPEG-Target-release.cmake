...
########## COMPONENTS TARGET PROPERTIES Release ########################################
    ...
    ########## COMPONENT JPEG::JPEG #############
        ...
        set(libjpeg-turbo_JPEG_JPEG_FRAMEWORKS_FOUND_RELEASE "")
        conan_find_apple_frameworks(libjpeg-turbo_JPEG_JPEG_FRAMEWORKS_FOUND_RELEASE "${libjpeg-turbo_JPEG_JPEG_FRAMEWORKS_RELEASE}" "${libjpeg-turbo_JPEG_JPEG_FRAMEWORK_DIRS_RELEASE}")
        ...
        set_property(TARGET libjpeg-turbo_JPEG_JPEG_DEPS_TARGET
                     PROPERTY INTERFACE_LINK_LIBRARIES
                     $<$<CONFIG:Release>:${libjpeg-turbo_JPEG_JPEG_FRAMEWORKS_FOUND_RELEASE}>
                     $<$<CONFIG:Release>:${libjpeg-turbo_JPEG_JPEG_SYSTEM_LIBS_RELEASE}>
                     $<$<CONFIG:Release>:${libjpeg-turbo_JPEG_JPEG_DEPENDENCIES_RELEASE}>
                     APPEND)

        ####### Find the libraries declared in cpp_info.component["xxx"].libs,
        ####### create an IMPORTED target for each one and link the 'libjpeg-turbo_JPEG_JPEG_DEPS_TARGET' to all of them
        conan_package_library_targets("${libjpeg-turbo_JPEG_JPEG_LIBS_RELEASE}"
                                      "${libjpeg-turbo_JPEG_JPEG_LIB_DIRS_RELEASE}"
        ...
        set_property(TARGET JPEG::JPEG
                     PROPERTY INTERFACE_LINK_LIBRARIES
                     $<$<CONFIG:Release>:${libjpeg-turbo_JPEG_JPEG_OBJECTS_RELEASE}>
                     $<$<CONFIG:Release>:${libjpeg-turbo_JPEG_JPEG_LIBRARIES_TARGETS}>
                     APPEND)

        if("${libjpeg-turbo_JPEG_JPEG_LIBS_RELEASE}" STREQUAL "")
            # If the component is not declaring any "cpp_info.components['foo'].libs" the system, frameworks etc are not
            # linked to the imported targets and we need to do it to the global target
            set_property(TARGET JPEG::JPEG
                         PROPERTY INTERFACE_LINK_LIBRARIES
        ...
        set_property(TARGET JPEG::JPEG PROPERTY INTERFACE_LINK_OPTIONS
                     $<$<CONFIG:Release>:${libjpeg-turbo_JPEG_JPEG_LINKER_FLAGS_RELEASE}> APPEND)
        set_property(TARGET JPEG::JPEG PROPERTY INTERFACE_INCLUDE_DIRECTORIES
                     $<$<CONFIG:Release>:${libjpeg-turbo_JPEG_JPEG_INCLUDE_DIRS_RELEASE}> APPEND)
        set_property(TARGET JPEG::JPEG PROPERTY INTERFACE_COMPILE_DEFINITIONS
                     $<$<CONFIG:Release>:${libjpeg-turbo_JPEG_JPEG_COMPILE_DEFINITIONS_RELEASE}> APPEND)
        set_property(TARGET JPEG::JPEG PROPERTY INTERFACE_COMPILE_OPTIONS
                     $<$<CONFIG:Release>:${libjpeg-turbo_JPEG_JPEG_COMPILE_OPTIONS_RELEASE}> APPEND)
